...
void cleanup(mainobj &mainset, int *img, int &sentinel, __m256 &four, __m256i &one, __m256i &eight, __m256i &zero) {
  ...
  mainset.iters.vec = _mm256_sub_epi32(mainset.iters.vec, _mm256_and_si256(_mm256_and_si256(_mm256_cmpgt_epi32(mainset.iters.vec, maxitervec), mainset.lanespeed.vec), eight));
  ...
  for (int i=0; i<8; i++) {
    ...
    if (mainset.iters.lanes[i] >= mainset.maxiter || mainset.magcmp.lanes[i] == -1) {
