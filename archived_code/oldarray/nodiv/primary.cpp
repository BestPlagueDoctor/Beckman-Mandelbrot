...
int main(int argc, char **argv) {
  ...
  */
  ...
  //free memory
  //calc
  Image img = Image(yres);
  //printcoord(img, xres, yres, res);
  imgmandel(maxiter, Image &img, res, xres, yres);
  rescale_colors_pgm(res, Image &img, &maxiter);
  write_pgm(filename, Image &img, xres, yres, res, maxiter);
  return 0;
  ...
  //WORK IN PROGRESS, SEE VECTOR BRANCH
}
